...
const doEnemyMovement = (enemy) => {
  ...
  const moveCharacter = (position, direction) => {
    ...
    let walkIndex = store.getState().enemy4.walkIndex + 1;
  ...
  const attemptMove = (direction) => {
    const oldPosition = store.getState().enemy4.position;
  ...
  // const getEnemyMove = () => {
  //   const playerPosition = store.getState().player.position;
  //   const enemyPosition = store.getState().enemy4.position;
  ...
  //   else return 1;
  // }
  let enemyInterval = setInterval(handleKeyDown, 200);
